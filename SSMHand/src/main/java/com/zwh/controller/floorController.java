...
public class floorController {
    ...
    public Map<String,Object> getList(@RequestParam(value = "name",required = false,defaultValue = "")String name,
        ...
        List<Floor> floorList = floorService.getFloorList(queryMap);
        int count = floorService.getCount(queryMap);
        result.put("rows",floorList);
    ...
    public Map<String,String> insertFloor(Floor floor, HttpServletRequest request) {
        ...
        logService.insertLog(role.getName() + admin.getUsername() + " 添加楼层信息成功！");
        return  result;
    }
    ...
    public Map<String,String> updateFloor(Floor floor,HttpServletRequest request) {
        ...
        User admin = (User)request.getSession().getAttribute("admin");
        Role role = (Role)request.getSession().getAttribute("role");
        logService.insertLog(role.getName() + admin.getUsername() + " 更新楼层信息成功！");
        return  result;
    }
    ...
    public Map<String,Object> deleteFloor(@RequestParam(value = "id",required = false) Integer id,
        ...
        try{
            if(floorService.deleteFloor(id) > 0) {
                result.put("success",false);
                result.put("msg","删除失败！");
                logService.insertLog(role.getName()+admin.getUsername() + " 删除楼层信息失败！");
                return result;
            }
        }catch (Exception e){
            result.put("success",false);
        ...
        logService.insertLog(role.getName()+admin.getUsername()+" 删除楼层信息成功！");
        return result;
    }
