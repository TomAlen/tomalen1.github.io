...
public class AccountController {
    ...
    public Map<String,Object> insertAccount(Account account,HttpServletRequest request) {
        ...
        if(accountService.insertAccount(account) > 0) {
            result.put("success",false);
            result.put("msg","添加失败！");
            logService.insertLog(role.getName() + admin.getUsername() + " 添加客户信息失败！");
            return result;
        }
        result.put("success",true);
        logService.insertLog(role.getName() + admin.getUsername() + " 添加客户信息成功！");
        return result;
    }
    ...
    public Map<String,Object> updateAccount(Account account,HttpServletRequest request) {
        ...
        if(accountService.updateAccount(account) > 0) {
            result.put("success",false);
            result.put("msg","添加失败！");
            logService.insertLog(role.getName() + admin.getUsername() + " 更新客户信息失败！");
            return result;
        }
        result.put("success",true);
        logService.insertLog(role.getName() + admin.getUsername() + " 更新客户信息成功！");
        return result;
    }
    ...
    public Map<String,Object> deleteAccount(Integer id,HttpServletRequest request) {
        ...
        try{
            if(accountService.deleteAccount(id) > 0) {
                result.put("success",false);
                result.put("msg","删除失败！");
                logService.insertLog(role.getName() + admin.getUsername() + " 删除客户信息失败！");
                return  result;
            }
        }catch (Exception e){
            result.put("success",false);
            result.put("msg","该客户下存在订单信息，请先删除该客户下的所有订单信息！");
            logService.insertLog(role.getName() + admin.getUsername() + " 删除客户信息失败，该客户下存在订单信息，请先删除该客户下的所有订单信息！！");
            return  result;
        }
        result.put("success",true);
        logService.insertLog(role.getName() + admin.getUsername() + " 删除客户信息成功！");
        return result;
    }
