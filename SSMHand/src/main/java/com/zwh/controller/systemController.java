...
import com.zwh.util.CpachaUtil;
import com.zwh.util.MenuUtil;
...
public class systemController {
    ...
    public Map<String,Object> loginActice (User user,String cpacha,HttpServletRequest request) {
        ...
        if(loginCpacha == null){
            ...
            logService.insertLog("用户"+user.getUsername()+"会话超时，重新输入验证码！");
            return result;
        }
        if(! cpacha.toUpperCase().equals(loginCpacha.toString().toUpperCase())) {
            result.put("success",false);
            result.put("msg","验证码错误！");
            logService.insertLog("用户"+user.getUsername()+"输入验证码错误！");
            return result;
        }
        ...
        if(ByUser == null) {
            ...
            logService.insertLog("用户"+user.getUsername()+"登录时，该用户不存在！");
            return result;
        }
        //判断传入的密码与数据库中的密码是否一致
        if(! ByUser.getPassword().equals(user.getPassword())) {
            result.put("success",false);
            result.put("msg","密码错误！");
            logService.insertLog("用户"+user.getUsername()+"登录时，密码错误！");
            return result;
        }
        ...
        result.put("msg","登录成功！");
        logService.insertLog(role.getName()+user.getUsername()+"登录成功！");
        return result;
    }
    ...
    public Map<String,Object> EditPassword(String oldPassword,String newPassword, HttpServletRequest request) {
        ...
        logService.insertLog(role.getName()+":"+user.getUsername()+"修改密码成功！");
        return result;
    }
    /**
     *
